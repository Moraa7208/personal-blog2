                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <x-input id="tags" class="block w-full mt-1" name="tags" type="text" value="{{ old('tag_name', $tags) }}"/>
                            ...
                            <select name="category" id="category" class="block w-full mt-1">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                            @if (in_array($category->id, $post->category->pluck('id')->toArray())) selected @endif>{{ $category->category_name }}</option>
                                @endforeach
                            </select>
                            @error('category')
                            <span class="font-medium text-red-600" role="alert">{{ $message }}</span>
